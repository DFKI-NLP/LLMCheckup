    ...
    for i in range(len(data)):
        ...
        for j in range(len(d_texts)):
            if dtype=='train':# or dtype=='val':
                samples.append((label2word[prev_label]+' [SEP] '+d_texts[j], d_labels[j], 1 if prev_label==-1 else 0))
            else:
                samples.append((d_texts[j], d_labels[j], 1 if prev_label==-1 else 0))
            prev_label = d_labels[j]
...
class CustomDAModel:
    ...
    def predict(self, X):
        ...
        prefix_labels = self.model.make_prefix_labels([-1])[0]
        prev_ids = prefix_labels['input_ids']
        prev_mask = prefix_labels['attention_mask']        
        for b_input in X:
            print(b_input)
            b_input = model.tokenizer.encode_plus(b_input, return_tensors='pt')
            input_ids = b_input['input_ids']#.to(device)
            input_mask = b_input['attention_mask']#.to(device)
            input_ids = model.prepend_label(prev_ids, input_ids).to(self.device)
            input_mask = model.prepend_label(prev_mask, input_mask).to(self.device)
            model.zero_grad()
            with torch.no_grad():
                ...
                prefix_labels = self.model.make_prefix_labels(prediction)[0]
                prev_ids = prefix_labels['input_ids']
                prev_mask = prefix_labels['attention_mask']
    ...
    def predict_proba(self, X):
        ...
        for b_input in X:
            ...
            with torch.no_grad():
                ...
                prediction = np.argmax(result.detach().cpu().tolist(), axis=1)
                prefix_labels = self.model.make_prefix_labels(prediction)[0]
                prev_ids = prefix_labels['input_ids']
                prev_mask = prefix_labels['attention_mask']
...
class DADataset:
    def __init__(self, samples):
        ...
        self.input_masks = []
        self.labels = []
        self.is_dialog_start = []
        
        for sample in samples:
            sample_text, label, is_start = sample
            ids, mask = self.get_id_with_mask(sample_text)
            self.input_ids.append(ids)
            self.input_masks.append(mask)
            self.labels.append(torch.tensor(label))
            self.is_dialog_start.append(torch.tensor(is_start))
    ...
    def __getitem__(self, idx):
        if self.labels is not None:
            ...
            label = None
        return self.input_ids[idx].squeeze(), self.input_masks[idx].squeeze(), self.is_dialog_start[idx], label
...
class DANetwork(nn.Module):
    ...
    def make_prefix_labels(self, labels):
        word_labels = ['[CLS] '+self.label2word[lbl]+' [SEP] ' for lbl in labels]
        return [self.tokenizer.encode_plus(wordlbl, add_special_tokens=False, return_tensors='pt') for wordlbl in word_labels]
        
    def prepend_label(self, prev_label_ids, text_ids):
        len_limit = self.max_seq_length - len(prev_label_ids) - 1 # -1 because of [CLS]
        text_with_label = torch.cat((prev_label_ids, text_ids[:,1:len_limit]), dim=1)
...
def train(epochs, model, lr, criterion, optimizer, scheduler, train_dataloader, dev_dataloader, device):
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_dataloader, 
            ...
            #print('>>>>>>>', model.tokenizer.batch_decode(input_ids, skip_special_tokens=True))
            #print(label_ids,'\n')
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(dev_dataloader, 
                                 ...
                                 desc=f'Train epoch {epoch+1}/{epochs}'):                    
                if prev_ids is None or batch[2][0].item()==1: # start of a dialogue
                    prefix_labels = model.make_prefix_labels([-1])[0]
                    prev_ids = prefix_labels['input_ids']
                    prev_mask = prefix_labels['attention_mask']
                input_ids = model.prepend_label(prev_ids, batch[0]).to(device)
                input_mask = model.prepend_label(prev_mask, batch[1]).to(device)
                ...
                prediction = np.argmax(logits, axis=1)
                prefix_labels = model.make_prefix_labels(prediction)[0]
                prev_ids = prefix_labels['input_ids']
                prev_mask = prefix_labels['attention_mask']
...
def evaluate(model, test_dataloader, device):
    ...
    for b_i, b_input in enumerate(test_dataloader):
        if prev_ids is None or b_input[2][0].item()==1:
            prefix_labels = model.make_prefix_labels([-1])[0]
            prev_ids = prefix_labels['input_ids']
            prev_mask = prefix_labels['attention_mask']     
        input_ids = model.prepend_label(prev_ids, b_input[0]).to(device)
        input_mask = model.prepend_label(prev_mask, b_input[1]).to(device)
